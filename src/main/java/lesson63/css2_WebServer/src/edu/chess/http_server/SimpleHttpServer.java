package lesson63.css2_WebServer.src.edu.chess.http_server;
...
public class SimpleHttpServer {
    ...
    public void launch () {
        ...
        while (true) {
            try {
                Socket s = ss.accept();
                /* Console */System.out.println("[Клиент принят]");
    ...
    boolean timeOutHttpServer (long timeStartServer) {
        ...
        if (timeNow > (timeStartServer + 6000000)) { return true; }
        return false;
    }
    ...
    private static class SocketProcessor implements Runnable {
        ...
        public void run () {
            /* Console */System.out.println("[Сервер HTTP получил запрос]");
            try {
                // *************************************************************
                ...
                ResourceToBytesFromFiles resourceContent = new ResourceToBytesFromFiles();
                byte[] responseBody = resourceContent.getResource(requestResourceName);
                writeResponse(responseBody, fileType);
                // *************************************************************
            } catch (Throwable e) {
        ...
        /**
         * Этот метод формирует ответ клиенту, в виде шоловы ответа и
        ...
        private void writeResponse (byte[] resourse, String mimeType) throws Throwable {
            ...
            response.append("Server: YarServer/+IlyaBeetle\r\n");
            response.append("Content-Type: " + mimeType + "\r\n");
            response.append("Content-Length: " + resourse.length + "\r\n");
            ...
            os.write(response.toString()
                    .getBytes(/*
                               * нужна ASCII-совместимая кодировка -
